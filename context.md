...
- Exchanges are available from POS receipt lookup and Admin POS Invoices. Returned and replacement stock, settlement, ledger entries, receipt creation, and unpaid commission cancellation run in one database transaction. Desktop exchanges persist in SQLite and synchronize idempotently after reconnect.
- Commission amounts are never negative. Refunds, voids, and exchanges cancel only unpaid earned commission through `cancelledAmount`; already-paid commission remains paid. Replacement items earn commission only when attributed to an active employee.
- Ledger entries can reference a vendor. Purchase and purchase-correction entries are linked automatically; manual expense/adjustment entries may select a vendor. The UI offers Main Ledger and Vendor Ledgers with date filters and running balances. Vendor deletion preserves ledger history through `ON DELETE SET NULL`.
- Receipts use a 72 mm page, approximately 66 mm content width and 3 mm padding. The heading is exactly `BILAL GARMENTS`. Every new and backfilled receipt has a collision-safe `BI-XXXX` Code 128 lookup value while existing invoice, receipt, and sale numbers remain unchanged.
...
**Deployment completed:** The user authorized deployment, superseding the local-only hold below. Application commit `ad06c1c` and test-isolation correction `e38c92f` are pushed to `main`. GitHub Build Verification `33763405286` passes all gates. Live frontend `/assets/index-DTquDUIv.js`, readiness/catalog HTTP 200, and the new short-barcode API were verified without business-data writes. Desktop 0.3.4 is published: older 0.3.3 clients receive `available: true`; 0.3.4 receives `false`; installer HEAD is 200 with 105,225,988 bytes and SHA-256 `948494745534348ae1055e30dc8ecc32ed3177bcfd23860f69442a3eb8d9a457`. No local installation or physical scanner test was performed. Commission reversal removal remains unimplemented pending a compensation-rule answer.

**Observed Hostinger release-file loss:** The follow-up code redeployment removed the first successful desktop upload (`published: false`, missing metadata/installer). Republished after all pushes and verified the feed/download successfully. Release storage is NOT proven persistent across redeployments; publish installers after the final web deployment until durable storage is configured. These final evidence edits in `context.md` and `docs/developer/qa-report.md` are intentionally local/uncommitted to avoid another automatic deployment. Application/test code is fully pushed. Do not blindly push documentation and assume the installer remains published; verify or republish afterward. The local-only notes below are historical.

**Historical commission note (superseded on 4 September):** Clarification was previously pending. The approved rule is now implemented: cancel unpaid earned commission without negative rows and preserve already-paid commission. See the 4 September release section above.

**Latest local-only barcode correction:** New generated barcodes use `AA-0000` format (two letters and four digits, plus a hyphen; for example `BA-1234`). Existing product/variant barcodes are not silently changed. Product editing has confirmed short-code replacement actions; Save and reprint are required, and SKUs/stock/size/color/QR remain intact. Sticker rendering uses Code 128 SVG at fixed physical dimensions with quiet zones and shared styles for every template; oversized codes are blocked instead of compressed. Focused backend and three fixture browser tests, build and lint pass. See `docs/developer/qa-report.md` for exact evidence and physical-printer acceptance. Phone-screen failure on the MS7120 laser scanner is not a valid label-quality test. These changes and POS fixes remain **local only**, not deployed or published; no production catalog codes were converted. The unsigned local installer `desktop/dist/BilalRMS-Setup-0.3.4.exe` was rebuilt (105,225,988 bytes); its hash is in the QA report. Release the matching backend with the frontend/desktop changes, not the installer alone.

**Later local-only POS fix:** Live read-only diagnosis found 27 active bootstrap products. `T-shirt 0002` exists with zero stock; the POS hid it as not found. Product-name submission for `COTTON JEANS 0002` incorrectly chose the first out-of-stock variant. Exact variant barcode `COTTONJEAN-BLA-24` worked. Local changes in `src/lib/pos-catalog.ts` and `src/routes/pos.tsx` add catalog browsing/refresh, explicit out-of-stock states, safe variant selection for shared names/codes, supplier-code lookup, and stable one-scan/one-add keyboard handling. Queued stock is preserved until sync succeeds. Eight fixture web/desktop-renderer tests pass; production frontend build and focused lint pass. Full frontend TypeScript has unrelated existing errors, documented in the QA report. Desktop version is prepared as `0.3.4`, but the user explicitly said **keep local for now**: do not push, deploy, publish, or install without a new request. The scanner's physical behavior is still untested; Orbit MS7120 reads 1D barcodes, not QR codes.
