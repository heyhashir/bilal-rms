...
**Deployment authorized:** The user has now requested deployment, superseding the local-only hold below. Releasing the completed POS/catalog, short-barcode/label and commission double-deduction fixes with desktop 0.3.4. Commission reversal removal remains unimplemented pending a compensation-rule answer. Publication and live checks are in progress; do not treat authorization as evidence that deployment has completed.

**Commission change awaiting business-rule clarification:** The user requested removal of reversal/negative entries on the commission screen. Asked whether refunds/voids should keep all original commission, or cancel unpaid commission without deducting paid commission; no answer yet. Do not infer the compensation rule or delete historical financial records. Independently fixed a confirmed report bug: EARNED already excludes PAID, so payable must not subtract PAID again. Backend build and `scripts/tests/commission-summary.test.mjs` pass for paid-only and mixed entries at total/employee/product levels. Commission reversal creation, listing and historical records are otherwise unchanged pending the answer. All changes remain local.
